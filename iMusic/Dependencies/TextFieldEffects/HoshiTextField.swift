...
@IBDesignable open class HoshiTextField: TextFieldEffects {
    ...
    var isCreditCardNumber : Bool = false
    /**
    ...
    fileprivate func layoutPlaceholderInTextRect() {
        ...
        placeholderLabel.frame = CGRect(x: isCreditCardNumber ? originX+40 : originX, y: textRect.height/2,
                                        width: placeholderLabel.bounds.width, height: placeholderLabel.bounds.height)
        activePlaceholderPoint = CGPoint(x:isCreditCardNumber ? placeholderLabel.frame.origin.x-40 : placeholderLabel.frame.origin.x, y: placeholderLabel.frame.origin.y - placeholderLabel.frame.size.height - placeholderInsets.y)
    ...
    override open func editingRect(forBounds bounds: CGRect) -> CGRect {
        return bounds.offsetBy(dx: isCreditCardNumber ? textFieldInsets.x+40 : textFieldInsets.x, dy: textFieldInsets.y)
    }
    
    override open func textRect(forBounds bounds: CGRect) -> CGRect {
        var xOffset = textFieldInsets.x
        if isCreditCardNumber {
            if text != "" {
                xOffset += 40
            }
        ...
        return bounds.offsetBy(dx: xOffset, dy: textFieldInsets.y)
    }
